...
| OAuthScopes | `List<OauthScopeCustomerAccessCode>` | List of scopes that apply to the OAuth token | `oauthScopes` | `getOauthScopes()` |



**Note:** Auth credentials can be set using `customerAccessCodeCredentials` in the client builder and accessed through `getCustomerAccessCodeCredentials` method in the client instance.
...
```java
String authUrl = client.getCustomerAccessCode().buildAuthorizationUrl(); // build auth url
...
try {
    ...
    OAuthToken token = client.getCustomerAccessCodeCredentials().fetchToken(authorizationCode);
    // re-instantiate the client with oauth token
    client = client.newBuilder()
            .customerAccessCodeCredentials(client.getCustomerAccessCodeModel().toBuilder()
                    .oauthToken(token)
                    .build())
            .build();
} catch (Throwable e) {
...
Scopes enable your application to only request access to the resources it needs while enabling users to control the amount of access they grant to your application. Available scopes are defined in the [`OauthScopeCustomerAccessCode`](../../doc/models/oauth-scope-customer-access-code.md) enumeration.
...
```java
if (client.getCustomerAccessCodeCredentials().isTokenExpired()) {
    try {
        OAuthToken token = client.getCustomerAccessCodeCredentials().refreshToken();
        // re-instantiate the client with oauth token
        client = client.newBuilder()
                .customerAccessCodeCredentials(client.getCustomerAccessCodeModel().toBuilder()
                        .oauthToken(token)
...
```java
// store token
httpSession.setAttribute("access_token", client.getCustomerAccessCodeCredentials().getOauthToken());
```
...
public class MainController {
    ...
    public void authcallback(HttpSession session, @RequestParam(required = false) String code,
        ...
        if (code == null) {
            ...
            synchronized (client) {
                authUrl = client.getCustomerAccessCodeCredentials().buildAuthorizationUrl();
            }
            ...
            synchronized (client) {
                token = client.getCustomerAccessCodeCredentials().fetchToken(code);
                // re-instantiate the client with oauth token
                client = client.newBuilder()
                        .customerAccessCodeCredentials(client.getCustomerAccessCodeModel().toBuilder()
                                .oauthToken(token)
